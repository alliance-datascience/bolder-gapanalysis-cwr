...
prepare_input_data <- function(data_path = choose.files( caption = "Select a valid .csv file"), 
  ...
  message("Preview Data base \n")
  print(head(data))
  
  
  #in case of col_number empty request a value from the user
  if(is.null(col_number)){
    warning("please enter the column number of response variable (Species name)", immediate. = TRUE, noBreaks. = T)
    col_number <- readline(prompt="Enter column number of response variable  (Species name): ") 
  }
 ...
 predictable <- ifelse(!is.na(data$Y), FALSE, TRUE)
  ...
  message("Removing coordinates on Oceans/Seas \n")
  ...
  #climate extraction
  current_clim_layer_generic <- lapply(generic_rasts, terra::rast)
  #current_clim_layer_sp      <- lapply(sp_rasts, terra::rast)
  #current_clim_layer         <- stack(c(current_clim_layer_generic, current_clim_layer_sp))
  current_clim_layer <- terra::rast(current_clim_layer_generic)
  ...
  clim_table2 <- terra::extract(current_clim_layer,original_data[, c("Longitude", "Latitude")])
