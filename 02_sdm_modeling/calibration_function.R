    ...
    tryCatch(expr = {
      ...
      if(use.maxnet){ 
        ...
        if(!file.exists(paste0(sp_Dir, "/calibration.csv"))){
          calibration <- enmSdmX::trainMaxNet(data = data, regMult = seq(0.5, 3, 0.5), out = 'tuning', verbose = T)
        ...
        if(!file.exists(paste0(sp_Dir, "/calibration.csv"))){
        calibration <- enmSdmX::trainMaxEnt(data = spData[,c(1,4:ncol(spData))], regMult = seq(0.5, 3, 0.5), out = 'tuning', verbose = FALSE, jackknife = F)
        write.csv(calibration, paste0(sp_Dir, "/calibration.csv"), quote = F, row.names = F)
        }else{
          cat("Calibration File already created, importing it \n")
          calibration <- read.csv(paste0(sp_Dir, "/calibration.csv"))
