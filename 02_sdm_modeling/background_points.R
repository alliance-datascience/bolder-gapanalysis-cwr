...
NAFiltered <- function(d = "potato", occName = "ajanhuiri"){
...
pseudoAbsences2 <- function(xy, background, exclusion.buffer = 0.0166, tms = 10, coord.sys = crs(mask)){
  ...
  terra::crs(presences) <- coord.sys
  spol <- terra::buffer(presences,width = exclusion.buffer)#spol <- rgeos::gBuffer(presences, width = exclusion.buffer)
...
pseudoAbsences_generator <- function(file_path, clsModel, overwrite = F, correlation = 0, pa_method = "ntv_area_ecoreg", rm_bg_region = NULL ){
  ...
  current_clim_layer_generic <- lapply(list.files(climDir, pattern =  ".tif$", full.names = T),  terra::rast)
  ...
  #current_clim_layer_sp      <- lapply(list.files(paste0(baseDir, "/input_data/by_crop/", crop, "/raster/", region), pattern = ".tif$", full.names = T), terra::rast)
  #current_clim_layer_sp      <- lapply(current_clim_layer_sp , function(aa){names(aa) <- gsub(".tif$", "", basename(aa@file@name)); return(aa)})
  all_clim_layer             <- terra::rast(current_clim_layer_generic)#rast(c(current_clim_layer_generic, current_clim_layer_sp))
  
  # remove undesirable layers
  layers_in <- terra::names(all_clim_layer)[ !tolower(terra::names(all_clim_layer)) %in% c("yield", "production", "harvested.area", "ethnicity", "monthCountByTemp10") ]
  ...
  if (!file.exists(outBackName) | overwrite){
    ...
    message("Processing:", paste(occName), "\n")
    ...
    spData$cellID <-NA
    spData$cellID <-terra::extract(mask,spData[,c("Longitude","Latitude")],cellnumbers=TRUE) #SpatialPoints(cbind(spData$Longitude, spData$Latitude))
    ...
    message("Creating random Pseudo-absences points using: ", pa_method, " method \n")
    # 
    ...
    #   } else {
    #     ntv_area <- terra::rast(paste0(input_data_dir, "/by_crop/", crop, "/native_area/", occName, "/native_area_", occName,".tif"))
    #   }
    #   
    #   climLayers <- terra::crop(current_clim_layer, raster::extent(ntv_area))
    #   climLayers <- terra::mask(climLayers, ntv_area)
    #   
    #  
    #   
    ...
    #   random_bg <- pseudoAbsences2(xy = unique(spData[,c("Longitude","Latitude")]), background = unsuit_bg$Absences, exclusion.buffer = 50000, tms = 10, coord.sys = crs(current_clim_layer))
    ...
    if(pa_method == "ecoreg"){
      ...
      elu[!(elu[] %in% regions)] <- NA # Exclude ecoregions that are not in occurrence data
      elu     <- terra::crop(elu,terra::ext(mask))
      elu     <- terra::mask(elu, mask)
      
      climLayers <- terra::crop(current_clim_layer, elu)
      climLayers <- terra::mask(climLayers, elu)
      
      
      
      unsuit_bg <- OCSVMprofiling2(xy = unique(spData[,c("Longitude","Latitude")]), varstack = climLayers, rm_bg_region = rm_bg_region)
      random_bg <- pseudoAbsences2(xy = unique(spData[,c("Longitude","Latitude")]), background = unsuit_bg$Absences, exclusion.buffer = 5000, tms = 10, coord.sys = terra::crs(current_clim_layer)) #0.083*5 for rgeos
      
      bg_spPoints  <- sp::SpatialPoints(coords = random_bg[random_bg$Status == 0, c("Longitude", "Latitude")])
      sp::proj4string(bg_spPoints)<- terra::crs(mask)
      bg2 <- terra::vect(bg_spPoints)
      terra::writeVector(bg2, paste0(input_data_dir, "/by_crop/", crop, "/", level, "/", occName, "/",region,"/background/background_", occName, ".shp"), overwrite = TRUE)
      
      nSamples <- nrow(random_bg[random_bg$Status == 0, c("Longitude", "Latitude")])
      message(nSamples, " pseudo-absences generated for n =", nrow(unique(spData[,c("Longitude","Latitude")])), " presences\n")
    ...
    ex_raster_env <- as.data.frame(terra::extract(all_clim_layer, xranSample))
    ex_raster_env$ID <- NULL
    z             <- cbind(id = 1:nrow(xranSample), species = occName, status = 0, xranSample, ex_raster_env)
    z             <- z[complete.cases(z),]
    message(nrow(z), " pseudo-absences ready to use\n")
    occ           <- z
    #occ$ID <- NULL
    # Preparing samples
    occSample        <- unique(spData[,c("Longitude", "Latitude")])
    names(occSample) <- c("lon", "lat")
    occ_env_data     <- as.data.frame(terra::extract(all_clim_layer, occSample))
    ...
    if(correlation == 0){
      message("Ommiting the correlation approach\n")
      swdSample <- swdSample_Complete
    }
    
    # Using choose variables algorithms (Correlation, VIF, or PCA + VIF)
    if(correlation == 1){
      message("Using Pearson correlation approach\n")
    ...
    if(correlation == 3){
      message("Using PCA + VIF approach","\n")
