...
# Old function
# message("Running modeling approach\n")
...
sdm_maxnet_approach_function <- function(occName      = occName,
  ...
  message("Loading environmental raster files \n")
  ...
  clim_layer    <- lapply(paste0(climDir, "/", var_names[clim_vars], ".tif"), terra::rast)
  if(any(generic_vars)){
    generic_layer <- lapply(paste0(clim_spReg,"/", var_names[generic_vars],".tif"), terra::rast)
    clim_layer  <- c(clim_layer, generic_layer)
    clim_layer  <- terra::rast(clim_layer)
  }else{
    clim_layer  <- terra::rast(clim_layer)
  }
 
  
  message("Initializing MAXNET model fitting throught cross validation. \n ")
  
  #split data  in nFolds for cross - validation process
  cvfolds <- modelr::crossv_kfold(spData, k= nFolds)
  message("Number of folds:", nFolds, "\n")
  #Do all sdm process
  sdm_results <- cvfolds %>% dplyr::mutate(.
                                            ...
                                            ,model_train = purrr::map2(.x = train, .y = .id, function(.x, .y){
                                              ...
                                              message("Training MAXNET model for fold ", .y, ", all presence points added to background \n")
                                            ...
                                            , predictions_train = purrr::pmap(list(.x = model_train, .y = .id, .z = train), function(.x, .y, .z){
                                              message("Predicting train data for fold", .y, "\n")
                                              train <- as.data.frame(.z)
                                              predictions <- enmSdmX::predictMaxNet(model = .x, newdata = train[, -1], type = "cloglog")
                                            ...
                                            ,AUC_train = purrr::map2(.x = predictions_train, .y = .id, function(.x, .y){
                                              message("Calculating AUC_train for model", .y,"\n")
                                            ...
                                            ,evaluation_train = purrr::map2(.x = predictions_train, .y = .id, function(.x, .y){
                                              ...
                                              message("Calculating optimal threshold for model", .y, "\n")
                                              croc <- pROC::roc(response = .x$obs, predictor = .x$pred)
                                            ...
                                            , predictions_test = purrr::pmap(list(.x = test, .y = model_train, .z = .id), function(.x, .y, .z){
                                              ...
                                              message("Using test data to predict model", .z," \n")
                                              test <- as.data.frame(.x)
                                              predictions <- enmSdmX::predictMaxNet(model = .y,newdata = test[, -1],type = "cloglog")
                                            ...
                                            , AUC = map2(.x = predictions_test, .y = .id, function(.x, .y){
                                              message("Calculating AUC for model", .y,"\n")
                                              croc <- pROC::roc(response = .x$obs, predictor = .x$pred)
                                            ...
                                            , evaluation_test = pmap(list(.x = evaluation_train, .y = .id, .z = predictions_test), function(.x, .y, .z){
                                              ...
                                              , nAUC = purrr::pmap(list(.x = train, .y = test, .z = .id), function(.x, .y, .z){
                                                message("calculating AUC from NULL model", .z,"\n")
                                              ...
                                              , cAUC = purrr::pmap(list(.x = AUC, .y = nAUC, .z = .id), function(.x, .y, .z){
                                                message("Calculating AUC correction using NULL model", .z, " \n")
                                              ...
                                              , do.projections =  purrr::pmap(list(.x = model_train, .y = .id, .z = evaluation_train) ,function(.x, .y, .z){
                                                ...
                                                message(">>> Proyecting MAXNET model ", .y," to a raster object \n")
                                                r <- enmSdmX::predictMaxNet(newdata = clim_layer, model=.x, type = "cloglog", progress='text')
                                                terra::writeRaster(r, paste0(model_outDir,"/replicates/",occName,"_prj_rep-", .y,".tif"), overwrite = TRUE)
                                                #thresholding raster 
                                                #  if(!validation){
                                                #    r[which(r[] < .z$threshold)] <- 0
                                                # }
                                                terra::writeRaster(r, paste(model_outDir,"/replicates/",occName,"_prj_th_rep-", .y,".tif",sep=""), overwrite = TRUE)
                                                r <- terra::wrap(r)
                                                return(r)
                                              })
...
#reading files to calculate consensus metrics
prj_stk <- lapply(1:length(sdm_results$do.projections), function(i){
  x <- terra::unwrap(sdm_results$do.projections[[i]])
  #x[which(x[]==0)] <- NA
  return(x)
})

#calculate  mean, median and sd raster from replicates 
prj_stk <- terra::rast(prj_stk)
message("Calculating mean, median and sd for replicates \n")
terra::mean(prj_stk) %>% terra::writeRaster(., paste0(model_outDir,"/", occName, "_prj_mean.tif" ), overwrite = TRUE)
message("Mean raster calculated \n")
terra::median(prj_stk) %>% terra::writeRaster(., paste0(model_outDir,"/", occName, "_prj_median.tif" ), overwrite = TRUE)
message("Median raster calculated \n")
if(doSDraster){
  terra::stdev(prj_stk) %>% terra::writeRaster(., paste0(model_outDir,"/", occName, "_prj_std.tif" ), overwrite = TRUE)
  message("Sd raster calculated \n")
}


#pseudo_df <- pseudo_df[which(!is.na(pseudo_df$mean)),]
#presence_df <- presence_df[which(!is.na(presence_df$mean)),]
#trying a different approach (the sum of thresholded and them use where models match!)
# prj_stk_total <- list.files(paste0(model_outDir,"/replicates"),pattern = "_prj_th_rep")
# prj_stk_total <- lapply(1:length(prj_stk_total),function(i){
#   x <- terra::rast(paste0(model_outDir,"/replicates","/",prj_stk_total[[i]]))
#   x[which(x[]>0)] <- 1
#   return(x)
# })
# prj_stk_total <- terra::rast(prj_stk_total)
# sdm <- sum(prj_stk_total,na.rm = T)#/5
# sdm[which(sdm[]<5)] <- 0
# sdm[which(sdm[]==5)] <- 1



message("calculating evaluation for median raster \n")

#Getting raster median probabilities for pseudoabsences and presences respectively
sdm <- terra::rast(paste0(model_outDir,"/", occName, "_prj_median.tif"))

pseudo_df <- cbind(spData[which(spData[,1]==0),],
                   sum=terra::extract(x = sdm,y= spData[which(spData[,1]==0),c("lon","lat")])$median)
presence_df <- cbind(spData[which(spData[,1]==1),],
                     sum=terra::extract(x = sdm,y= spData[which(spData[,1]==1),c("lon","lat")])$median)
#evaluate model
eva <- dismo::evaluate(p=presence_df$sum, a=pseudo_df$sum)
#getting threshold
thr <- dismo::threshold(eva)

#Thresholding SDM using max sens + specificity
sdm[sdm[]>=thr[[2]]] <- 1
sdm[sdm[]<thr[[2]]] <- 0

terra::writeRaster(sdm, paste0(model_outDir,"/", occName, "_prj_median_th.tif" ), overwrite = TRUE)
# sdm[sdm[]>=thr[[2]]] <- 1
# sdm[sdm[]<thr[[2]]] <- 0

#adding thresholding values to pseudoabsences and presences
pseudo_df$THR <- terra::extract(x = sdm,y= spData[which(spData[,1]==0),c("lon","lat")])$median
presence_df$THR <- terra::extract(x = sdm,y= spData[which(spData[,1]==1),c("lon","lat")])$median

#calculating confusion matrix
a <- nrow(presence_df[which(presence_df$THR==1),])
b <- nrow(pseudo_df[which(pseudo_df$THR==1),])
c <- nrow(presence_df[which(presence_df$THR==0),])
d <- nrow(pseudo_df[which(pseudo_df$THR==0),])

#sensitivity and specificity
se <- a/(a+c)
es <- d/(b+d)
#minROC
#minROCdist = sqrt((1- se)^2 + (es -1)^2)
#calculating TSS    
TSS <- (se+es)-1
#saving evaluation matrix for the median ensemble!
eval_table <- data.frame(
           species = occName,
           var_names=paste(var_names,collapse = ";"),
           AUC = eva@auc,
           presences=nrow(presence_df),
           absences= nrow(pseudo_df),
           sensitivity= se,
           specificity=es,
           TSS = TSS,
           threshold= thr[[2]])
#Saving data frame
write.csv(eval_table,paste0(eval_sp_Dir,"/","evaluation.csv"),na = "",row.names = F)
######## calculate varImean######## calculate varImportance ########

if(varImp){

  sdm_results <- sdm_results %>% dplyr::mutate(., varImp = purrr::pmap(list(.x = train, .y = .id, .z = model_train), function(.x, .y, .z){
    message("Calculating var importance for model", .y, "... \n")
    ...
    for(k in 1:5){
      ...
      pred_rand <- data.frame(obs = p, pred = enmSdmX::predictMaxNet(model = .z , newdata = data_rand, type = "cloglog"))
...
message("Process Done... Saving results as .rds file in the path", paste0(sp_Dir, "/sdm.rds"), " \n")
...
#                                          , predictions_test = purrr::pmap(list(.x = test, .y = model_train, .z = .id), function(.x, .y, .z){
#                                            
#                                            message("Using test data to evaluate model", .z," \n")
#                                            test <- as.data.frame(.x)
